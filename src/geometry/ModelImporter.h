	...
	private:
		...
		std::vector<DataTriangle> processFirstMesh(aiMesh *mesh) {
			...
			for(unsigned int i = 0; i < mesh->mNumFaces; i++) {
				...
				triangles.emplace_back(vertices[0], vertices[1], vertices[2], normal, ci::ColorA(1,0,0,1));
			}
			return triangles;
		}

		/// Calculates triangle normal from its vertices.
		glm::vec3 calculateNormal(glm::vec3 *v) {  // hoping for correct direction
			glm::vec3 V1 = (v[1] - v[0]);
			glm::vec3 V2 = (v[2] - v[0]);
			glm::vec3 faceNormal;
		...
		/// Calculates triangle normal from its vertices with orientation of original (probably not surface) normals.
		glm::vec3 calculateNormal(glm::vec3 *v, glm::vec3 *n) {
			glm::vec3 faceNormal = calculateNormal(v);
			glm::vec3 vertexNormal = n[0] + n[1] + n[2];

			return (glm::dot(faceNormal, vertexNormal) < 0.0f) ? -faceNormal : faceNormal;
		}
